...
var internals = {
	...
	drawFn : function( currentForceLayout, current, app ) {
		...
		return function () {
			...
			internals.unPackPosition( shader, nodes )
			internals.unPackColor( shader, nodes )
	...
	unPackPosition : function( shader, nodes ) {
	...
	unPackColor : function( shader, nodes ) {
		
		var values = shader.attributes.color.value
		
		//Update the shader positions			
		for( let i=0; i < nodes.length; i++ ) {
			let node = nodes[i]
			
			values[i*3] = node.color[0]
			values[i*3+1] = node.color[1]
			values[i*3+2] = node.color[2]
		}
		
		shader.attributes.color.bufferData()
